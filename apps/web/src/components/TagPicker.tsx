...
async function lookupTags(q: string): Promise<TagOpt[]> {
  ...
  search.set("q", q);
  search.set("limit", "20");
  const path = `/admin/tags/lookup?${search.toString()}`;
  const data = await apiGet<{ items: TagOpt[] }>(path, "browser", undefined, token);
  return data.items;
}
...
export default function TagPicker({
  ...
  value: TagOpt[];
  onChangeAction: (next: TagOpt[]) => void;
  ...
  const [results, setResults] = useState<TagOpt[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const items = await lookupTags(dq);
  ...
  function add(t: TagOpt) {
    if (value.some((x) => x.id === t.id)) return;
    onChangeAction([...value, t]);
    setQ("");
    setOpen(false);
  }
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap gap-2">
        {value.map((t) => (
          <span key={t.id} className="border rounded px-2 py-1 text-sm inline-flex items-center gap-2">
            {t.name}
            <Button type="button" variant="plain" size="none" className="opacity-70 hover:opacity-100" onClick={() => remove(t.id)}>
              ×
            </Button>
          </span>
        ))}
        {value.length === 0 && <div className="text-sm opacity-60">No tags selected.</div>}
      </div>
